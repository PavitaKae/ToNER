...
def data_process(bam1,bam2,lib,case):
	samtool="samtools"   # Please put your samtools path here
	try:
	...
	if issort == "coordinate":
		...
		os.rename("../"+bam1,'../'+bam1.replace(".bam","_before_sort.bam"))
	...
	if issort2 == "coordinate":
		...
		os.rename("../"+bam2,'../'+bam2.replace(".bam","_before_sort.bam"))
...
def boxcox_data_transform(ratio,lib,p_value,pass_cut_off):
	...
	plt.savefig(lib+'_Box-Cox_transformation.svg', dpi=300, format='svg')
	plt.clf()
...
def find_empirical(ratio,percentile,lib):
	...
	plt.savefig(lib+'_top-rank.svg', dpi=300, format='svg')
	plt.clf()
	all_p_value={}	
...
def pie_plot(data,text,name_save,lib):
	...
	plt.savefig(name_save+'.svg', dpi=300, format='svg')
	plt.clf()
...
def input_gene_gff(input_folder):
	...
	for line in f:
		...
		if line[0]=='#':
			...
			locus_tag=""
			for text in info:
				if re.search('ID=',text,re.IGNORECASE):
					try:
...
def read_per_position(data1,data2,lib):
	...
	plt.savefig(lib+'_reads_per_Position.svg', dpi=300, format='###1.2 add svg')
	plt.clf()
...
def main():
	...
	parser.add_option("--ngff", dest="check_gff", default=True, action="store_false",
		help="add gene annotate to result even if gff error")
	...
	if annotate :
		if check_gff:
			...
			print_out("put annotate gene (not check gff) :\tgene "+annotate,False)
	else: 
